...
class GeneralDetailsViewModel @Inject constructor(
    ...
    var isFromAssessment = false
    val siteDetail = SiteDetails()
    val siteDetailSaved = MutableLiveData<Resource<Boolean>>()
    var accountSiteListLiveData = MutableLiveData<Resource<ArrayList<SiteEntity>>>()

    fun saveChosenSiteDetails() {
        viewModelScope.launch(dispatcherIO) {
            siteDetailSaved.postLoading()
            try {
                screeningRepository.saveChosenSiteDetail(siteDetail)
                siteDetailSaved.postValue(Resource(ResourceState.SUCCESS, true))
            } catch (e: Exception) {
                siteDetailSaved.postValue(Resource(ResourceState.SUCCESS, false))
            }
        }
    }
